<x-guest-layout>
    <div class="flex h-screen w-full items-center justify-center bg-gray-600">
        <div class="min-w-4xl  rounded bg-gray-50 px-6 pt-8 shadow-lg space-y-2">
            <div class="flex justify-center text-center">
                <x-branding />
            </div>
            <hr class="py-2">
            <div class="flex flex-col justify-center items-center gap-2 py-2">
                <h4 class="font-semibold text-sm W-3/4 text-center">{{ config('app.businessname') }}</h4>
                <p class="text-xs">{{ __('Payment Receipt') }}</p>
            </div>
            <div class="flex flex-col gap-3 border-b py-3 text-xs">
                <p class="flex justify-between">
                    <span class="text-gray-400">{{ __('Receipt No') }}.:</span>
                    <span>{{ $receipt->invoice_number }}</span>
                </p>
                <p class="flex justify-between">
                    <span class="text-gray-400">{{ __('Order Type') }}:</span>
                    <span>{{ __('Institute Participation') }}</span>
                </p>

                <p class="flex justify-between">
                    <span class="text-gray-400">Customer:</span>
                    <span class="text-sm font-bold">{{ $receipt->invoicee->full_name }}</span>
                </p>
            </div>
            <div class="flex flex-col gap-3 pb-6 pt-2 text-xs">
                <table class="w-full text-left">
                    <thead>
                        <tr class="flex">
                            <th class="w-full py-2 text-md">Product</th>
                            <th class="min-w-[44px] py-2 text-md">QTY</th>
                            <th class="min-w-[44px] py-2 text-md">Total</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr class="flex ">
                            <td class="flex-1 py-1 text-sm">{{ $receipt->edition->title }}</td>
                            <td class="min-w-[44px] text-sm">{{ __('1') }}</td>
                            <td class="min-w-[44px] text-sm">${{ $receipt->edition->price }}</td>
                        </tr>
                    </tbody>
                </table>
