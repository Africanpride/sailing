<x-app-layout>
    <x-backend-page-header model-name="editions " description="COSTrAD & editions" add-button="false" class="mx-4">
        <x-heroicon-o-user-group class="w-5 h-5 text-current" />
    </x-backend-page-header>


    <div class="flex justify-between items-center gap-x-4 px-4">
        <x-button class="w-full" type="button" onclick="Livewire.dispatch('openModal', { component: 'add-edition' })">
            {{ __('Add New Mother edition ') }}
        </x-button>
        <x-button class="w-full" type="button">
            <div onclick="Livewire.dispatch('openModal', { component: 'create-edition-edition' })"
